...
# load data
theData <- fread("pcl_sim_data.csv")
thresholds <- read.csv("thresholds_pcl.csv", header = TRUE, row.names = 1)
...
ordFacData <- mxFactor(theData[,c(1:20)], levels = 0:4, ordered = TRUE)#, exclude = 888) # if non-level values
...
# simulated thresholds for ordinal data (20 items)
thresholds <- matrix(unlist(thresholds), nrow = nrow(thresholds))
...
# exploratory factor analysis with raw data (using integer data, fa() requires numerica data), 1 to 5-factor models based on number of eigenvalues
...
# 3-factor oblique solution (crit BC, D, E)
# OpenMx
numFactors <- 3
...
items1fac <- c("pcl_1",  "pcl_2",  "pcl_3",  "pcl_4",  "pcl_5",
               "pcl_6", "pcl_7")
items2fac <- c("pcl_8",  "pcl_9",  "pcl_10",  "pcl_11",  "pcl_12", "pcl_13",  "pcl_14")
items3fac <- c("pcl_15",  "pcl_16",  "pcl_17",  "pcl_18",  "pcl_19", "pcl_20")


## for the models with >1 factors, do it manually (for now):
load1fac <- vars %in% items1fac
load2fac <- vars %in% items2fac
load3fac <- vars %in% items3fac

# free and fix the loading paths accordingly
freeFixed <- c(load1fac, load2fac, load3fac)

# check them
length(freeFixed[freeFixed == TRUE])
length(freeFixed[freeFixed == FALSE])

# set up OpenMx objects
...
FacCov            <- mxMatrix("Symm", numFactors, numFactors, values=c(1,0,0,1,0,1), free=c(F,T,T,F,T,F), name="FacCov")

obj <-     mxExpectationNormal(covariance="impliedCovs", means="Mean", dimnames = vars, thresholds="thresholdMatrix")
fitFun <- mxFitFunctionWLS(type = "DWLS") ## could do WLS (type = "DWLS) if taking too much with ML, and vars are ordinal
ordData <- mxData(observed=ordFacData, type='raw')
FullModel3dnonEFA <- mxModel("Full3dnonEFA", load, Ones, Mean, Errors, impliedCovs, threshDev, unitLower, thresholdMatrix, FacCov, obj, fitFun, ordData)
fitCFA3dnonEFA <- mxTryHardOrdinal(FullModel3dnonEFA)
summary(fitCFA3dnonEFA) 

checkIdentfitCFA3dnonEFA <- mxCheckIdentification(fitCFA3dnonEFA)


# lavaan
lvModelCFA3oblNonEFA <- c(paste0("f1 =~ ", paste(items1fac, collapse = " + ")),
...
lvFitCFA3oblNonEFA <- cfa(lvModelCFA3oblNonEFA, data = ordFacData, std.lv=TRUE, missing = "pairwise")#, auto.cov.lv.x=FALSE)
summlvFitCFA3oblNonEFA <- summary(lvFitCFA3oblNonEFA, fit.measures = TRUE, standardized = TRUE)
summlvFitCFA3oblNonEFA


# Measurement invariance
# model from CFA results
lvModelCFA3oblNonEFA_MI <- lvModelCFA3oblNonEFA

# configural
lvFit_config <- cfa(lvModelCFA3oblNonEFA_MI, data = ordFacData4MI, std.lv = TRUE, missing = "pairwise", group = "is_case")
...
# metric
lvFit_metric <- cfa(lvModelCFA3oblNonEFA_MI, data = ordFacData4MI, std.lv=TRUE, missing = "pairwise", group = "is_case", group.equal = "loadings")
summary(lvFit_metric, fit.measures = TRUE, standardized = TRUE)
fitMeasures(lvFit_metric, c("rmsea", "cfi", "tli"))

# scalar
lvFit_scalar <- cfa(lvModelCFA3oblNonEFA_MI, data = ordFacData4MI, std.lv=TRUE, missing = "pairwise", group = "is_case", group.equal = c("loadings", "intercepts"))
...
lvFit_strict <- cfa(lvModelCFA3oblNonEFA_MI, data = ordFacData4MI, std.lv=TRUE, missing = "pairwise", group = "is_case", group.equal = c("loadings", "intercepts", "residuals"))
...
length(unique(c(items1fac, items2fac, items3fac)))

modelIRT <- '
F1 = pcl_1, pcl_2, pcl_3, pcl_4, pcl_5, pcl_6, pcl_7
F2 = pcl_8, pcl_9, pcl_10, pcl_11, pcl_12, pcl_13, pcl_14
F3 = pcl_15, pcl_16, pcl_17, pcl_18, pcl_19, pcl_20
...
item_pars # a are discrimination for each item-factor, b are for difficulty
...
lvcfaFacFSsEBM <- lavPredict(lvFitCFA3oblNonEFA, method = "EBM") # EBM for empirical Bayes modal 
